...
/// Builds a 64-byte write report from the current device state.
///
/// Read response layout:  [FF 03 devId SEN FF SHO BLI FFS ...]
/// After copy to writeBuf: writeBuf[3]=devId, writeBuf[4]=SEN, writeBuf[5]=FF, ...
/// The device has no Slot byte in its read response, so the overlay uses
/// `writeBuf[ADDR + 1] = wire_value` — one past the read-buffer position.
/// Use `PwsProfile::to_params()` to obtain the (ADDR_*, wire_value) pairs.
...
/// Note: single-param writes do not work on the DD+ — use build_full_write_report.
#[allow(dead_code)]
